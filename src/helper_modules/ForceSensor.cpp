...
ForceMeasurement ForceSensor::getCalibratedForceMoment(const Vector3d& raw_force, const Vector3d& raw_moment) {
    ...
    Matrix3d R_world_to_link;  // rotate link frame to world frame 
    R_world_to_link = _robot->rotation(_link_name);
    Matrix3d R_world_to_sensor;  // rotate sensor frame to world frame 
    R_world_to_sensor = R_world_to_link * _T_link_to_sensor.linear();
    ...
    VectorXd vel = _robot->velocity6dInWorld(_link_name, _T_link_to_sensor.translation());
    VectorXd accel = _robot->acceleration6dInWorld(_link_name, _T_link_to_sensor.translation());
